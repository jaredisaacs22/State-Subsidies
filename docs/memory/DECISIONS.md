...
| 2026-07-12 | **Theme F-2 (console gate) pulled ahead of Theme B-1 (per-source golden fixtures)** | B-1 needs live payload capture across ~25 flaky sources — high cost, incremental value; the console gate closed the last open scar (LESSONS #12) cheaply by riding the existing boot-probe server. Deviation from the written order, recorded per doctrine | B-1 is next |
| 2026-07-12 | **Playwright config honors `PW_CHROMIUM_PATH`** for sandboxes with pre-provisioned Chromium; CI installs the version-matched browser via `npx playwright install chromium` | Version-matched download is correct in CI; local/sandbox environments may pin a system chromium instead | — |
| 2026-07-12 | **Vercel Analytics/SpeedInsights render only when `process.env.VERCEL` is set** | The injected `/_vercel/*` scripts 404 on any self-hosted boot (CI, local `next start`), guaranteeing console errors; production on Vercel is unchanged | If analytics ever move off Vercel |
