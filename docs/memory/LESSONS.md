...
| 5 | 2026-04-30 | Scraper failures logged as `status: "ok"` | Success-path code inside the `except` block overwrote the fail status | Fix in PR #51 — **but never test-pinned, and it regressed: see #16** |
| 16 | 2026-07-11 | **LESSONS #5 shipped a SECOND time** — a merge reintroduced the fail→ok overwrite into `_run_scrapers`, plus a stale-variable bug (failed source stamped "ok" with the *previous* source's row count; a first-scraper failure raised NameError and killed the whole run) and an unreachable duplicate `except` | The PR #51 fix was a fix without a pinned test — exactly the failure mode doctrine §0.3 predicts | Refactored to `_run_scraper_list` + 3 pytest pins (`tests/test_batch_gate.py::TestRunScraperListFailIsolation`): fail stays fail, one failure never kills the run, no stale counts |
...
| 12 | 2026-07-10 | Browse page crashed in production | Router crash surfaced by real traffic, not by any test — we have zero browser tests | **Fully pinned 2026-07-12:** `boot-probe` catches boot/render crashes; Playwright console-error gate (`tests/e2e/console-gate.spec.ts`, zero tolerated errors) rides the same CI job. First run immediately caught a real defect: sitewide favicon 404 in every visitor's console |
...
*(none as of 2026-07-12 — all 16 scars carry pins)*
