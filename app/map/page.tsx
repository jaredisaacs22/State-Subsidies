...
export default function MapPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* ── Dark hero header ─────────────────────────────────────────── */}
      <div className="hero-section text-white px-4 sm:px-6 lg:px-8 pt-10 pb-8">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-2xl font-extrabold tracking-tight mb-1">Incentives by State</h1>
          <p className="text-white/55 text-sm mb-6">
            {totalPrograms > 0 ? (
              <><span className="font-semibold text-white/80">{totalPrograms.toLocaleString()}</span> active programs across all <span className="font-semibold text-white/80">{totalStates}</span> states — click any state to explore.</>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {/* Map */}
        <div className="lg:col-span-3 bg-white rounded-2xl border border-forest-100 shadow-sm p-6">
          <USStateMap
            counts={counts}
            selected={selected}
      ...
      {Object.keys(counts).length > 0 && (
        <div className="mt-8">
          <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-4">All States — Program Count</h2>
